...
$(document).ready(function() {
    ...
    $(".la-option-validate").click(function(e) {
        ...
        console.log($("#la-currency-list").val());
        storage.set('currency', currency[$("#la-currency-list").val()]);
        storage.set('currencyIcon', $("#la-currency-list").val());
    ...
    storage.get('currency', function(current) {
        $.each(currency, function(key, value) {
            $("#la-currency-list").append($("<option>").attr(current == value ? {
                'selected': 'selected'
            } : {}).html(key));
        });
    });
    ...
    storage.get('timeFormat', function(format) {
        $.each(timeFormat, function(key, value) {
            $("#la-time-format").append($("<option>").attr(format == value ? {
                'selected': 'selected'
            } : {}).html(key));
        })
    });
